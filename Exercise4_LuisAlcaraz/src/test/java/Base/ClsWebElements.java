...
public class ClsWebElements extends ClsBrowser
{
    ...
    public WebElement getWebElement(By by)
    {
        try
        {
            ...
            //ClsReport.fnLog(Status.PASS, "The Web Element: " + by.toString() + " was found as expected.", false);
            return pobjElement;
        }
        catch (NoSuchElementException pobjException)
        {
        	ClsReport.fnLog(Status.FAIL, "The Web Element: " + by.toString() + " was not found as expected. Exception: " + pobjException.getStackTrace(), true);
        	return null;
        }
    }
    ...
    public void deployableList (String listSelect, String listValue) throws InterruptedException
    {
    	Thread.sleep(2000);
    	WaitForLoad();
    	Select dpList = new Select (objDriver.findElement(By.xpath(listSelect)));
	    dpList.selectByVisibleText(listValue);
    }
    
    public void getInt (String pstrPrice, int budgetFix)
    {
    	pstrPrice = pstrPrice.replaceAll("MXN", "");
    	pstrPrice = pstrPrice.replaceAll(",", "");
    	pstrPrice = pstrPrice.replaceAll("$", "");
    	pstrPrice = pstrPrice.replaceAll(" ", "");
		
		ClsReport.fnLog(Status.PASS, "The final price is: " + pstrPrice , false);
		
		String Price = pstrPrice.substring(1, pstrPrice.length());
		
		int priceConvertion = Integer.parseInt(Price);
	
		if(priceConvertion < budgetFix) {
			
			ClsReport.fnLog(Status.PASS, "Prepare your luggage! ", false);
			
		} else {
			
			ClsReport.fnLog(Status.FAIL, "The price was $" + (priceConvertion-budgetFix) + " over your budget ", false);
